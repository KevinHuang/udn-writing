...
上面那份清單講的是**認證**（你是誰）。`apps/api` 另外有一類問題是**授權**
（你能碰哪些資料），而且它在這個 codebase 裡**已經出現四次**，形狀完全一樣：

> 把關寫在路由的某一處，但真正動資料的那一步沒有帶上它。

| 出現位置 | 症狀 |
|---|---|
| 資源 7 的三支批改方法 | 完全沒有授權。`returnFeedback` 最嚴重 —— 不可逆地發還別班的成績 |
| 四支 assignment 方法 | 收了 `user_id` 參數，但 `WHERE` 裡沒有用到它 |
| `POST /instructor/grading/:id`（AI 批改） | 先呼叫 AI 再把關，而且把關的回傳值（null）被忽略，照樣回 200 —— 別班學生的作文與批改結果已經回給呼叫者 |
| `GET /instructor/courses/:id/finalReports` | 路由取了 `userId` 卻沒有往下傳 |
| `SubmissionHelper.resetSubmissions()` | helper **連參數都沒有**。而且它後面還跟著一支有把關的，打別班時第一支成功、第二支被擋，留下半毀狀態 |

**為什麼型別檢查抓不到**：多給一個沒用到的參數不是錯誤，少一個 `AND` 也不是。
四次都是「程式跑得好好的，只是對錯的人也跑得好好的」。

### 這個專案的作法

1. **授權條件寫進 SQL 的 `WHERE`**，和寫入是同一個不可分割的敘述。
   不要「先 SELECT 檢查、再 UPDATE」—— 兩段式在中間有空隙，而這些
   操作多半是破壞性的。
2. **擁有權是 DAL 方法的必填參數**，不是路由裡的 `if`。
   改簽名的時候 `tsc` 會把所有呼叫端指出來 ——
   `getFinalReport` 那次就是這樣被找出來的。
3. **404 同時涵蓋「沒這一筆」與「不是你的」**，刻意不區分，
   否則回應本身就是一個可以枚舉 id 的探針。
4. **每一支都要有「別班 → 擋掉，而且資料原封不動」的測試。**
   只測 status code 不夠 —— `resetSubmissions` 那次，
   真正要釘住的是「別班的 `ref_user_id` 沒有變成負數」。

### 掃描方式

```bash
# 找出「取了 userId 卻沒往下用」的 handler
grep -n "userInfo.id" apps/api/src/routes/*.ts
```

逐一確認那個值有沒有真的進到 SQL 的 `WHERE`。**變數有被引用不等於有把關** ——
`resetSubmissions` 那次，`userId` 確實被下一行用到了，漏的是上一行。
