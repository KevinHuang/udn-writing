...
export const SCAN_CONFIG = {
   ...
   * 托盤縮圖的長邊。
   *
   * ⚠ 不要圖省事拿 ocrBlob（2000px）當縮圖來源 —— `<img>` 會把它解碼成
   *   約 11MB 的點陣圖，托盤放六張就 66MB，iOS Safari 會直接砍掉分頁。
   *   240px 解碼後只有 160KB 左右，而且產生它幾乎免費（樣式的 Mat
   *   在畫結果頁預覽時已經建好了）。
   */
  thumbLongSide: 240,
  /**
   * 一次最多收幾張稿紙。
   *
   * 一篇作文常見是 1–3 張，8 張已經很寬裕。上限的理由是記憶體與時間：
   * 8 張的原稿約 24MB，送辨識時每張還要轉成 1.33 倍大的 base64 字串，
   * 而逐張序列辨識 8 張大約要一分半，中途沒有退路。
   */
  maxScanPages: 8,
} as const;
